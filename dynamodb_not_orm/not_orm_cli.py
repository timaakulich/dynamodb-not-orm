...
async def create_empty(description: str = typer.Argument("auto")):
    ...
    migration_files = sorted(p for p in MIGRATIONS_DIR.glob("[0-9]*_*.py"))
...
async def migrate(
    migration_number: Optional[str] = typer.Argument(
        None, help="Migration number (e.g., 0001, 0002) or zero to unapply all"
    ),
    aws_region: str = typer.Option(...),
    environment: str = typer.Option(...),
    app_name: str = typer.Option(...),
    ...
    if not migration_files:
        typer.secho("No migration files found", fg=typer.colors.RED, err=True)
        return
    ...
    try:
        ...
        typer.secho(
            f"Could not read from {table_name} table.Make sure it exists.",
            fg=typer.colors.RED,
            err=True,
        )
        return
        ...
        if target_action is None:
            ...
            if int(target_migration) > current_highest:
                ...
                migrations_to_process = [
                    ...
                    if not m["is_applied"]
                    and int(m["number"]) <= int(target_migration)  # noqa
                ]
            else:
                action = "downgrade"
                migrations_to_process = [
    ...
    if not migrations_to_process:
        typer.secho(f"No migrations to {action}", fg=typer.colors.YELLOW)
        return
    ...
    for migration in migrations_to_process:
        ...
        try:
            ...
            typer.secho(
                f"Failed to {action} migration {migration['number']}: {e}",
                fg=typer.colors.RED,
                err=True,
            )
            return

    typer.secho(f"Migration completed successfully!", fg=typer.colors.GREEN)
